//*************** Constants ****************//
var MODULE_ID = '[ESQ_ROUTER]';
var logger = require('log4js').getLogger(MODULE_ID);
var GET_URL_STRUCTURE = "/:tenantId/api/netflow/:srcGrp/:destGrp";


//*************** Require External Modules ****************//
var responseHandler = require("./response_handler.js");
var requestHandler = require("./request_handler.js");
var NetflowRequest = require("./NetflowRequest.js");
var HttpStatus = require('http-status-codes');

// Setting routes according to the routing table
function setRoutes(app) {
    
	app.get(GET_URL_STRUCTURE, function(req, res) {

		var startOffset = req.query.start;
		var endOffset = req.query.end;
		var startTime = req.query.startTime;
		var endTime = req.query.endTime;
		if(!startOffset|| !endOffset || !startTime || !endTime) {
			res.writeHead(HttpStatus.BAD_REQUEST, {
				"Content-Type" : "application/json"
			});
			res.write("Bad Request: Please check your URL, the format should be - " +
				"<TenantId>/api/netflow/<SourceGroupName>/<DestGroupName>?start=<startOffset>" +
				"&end=<endOffset>&startTime=<StartTimeStamp>&endTime=<EndTimeStamp>");
			res.end();
		}
	
		netflowRequest = new NetflowRequest(req.params.tenantId, req.params.srcGrp, req.params.destGrp,
		 startOffset, endOffset, startTime, endTime);

		var err = false;
		//Calling the request handler to retrieve top searches 
		requestHandler.getResponse(netflowRequest, function(err, data, count){
			responseHandler.handleResponse(res,err,data,count);
		});	
			
	});
	
    
	function logErrors(err, req, res, next) {
			console.log("Error: ", err.stack);
			next(err);
		}
	
	function errorHandler(err, req, res, next) {
			var message = "URL: "+ req.url + " STACK: " + err.stack;
			res.status(500).send({status:500, message: message, type:'internal'});
		}
	
	app.use(logErrors);
	app.use(errorHandler);
}

//*************** EXPORTED PROPERTIES *****************//
exports.setRoutes = setRoutes;
